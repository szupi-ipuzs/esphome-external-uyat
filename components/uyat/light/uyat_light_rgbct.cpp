...
#include "uyat_light_rgbct.h"

namespace esphome::uyat {

UyatLightRGBCT::UyatLightRGBCT(Uyat *parent, Config config):
parent_(*parent),
...
void UyatLightRGBCT::setup_state(light::LightState *state) { state_ = state; }

void UyatLightRGBCT::write_state(light::LightState *state) {
...
void UyatLightRGBCT::on_dimmer_value(const float value_percent)
{
  ESP_LOGV(UyatLightRGBCT::TAG, "Dimmer of %s reported brightness: %.4f", this->get_name().c_str(), value_percent);
  if (this->state_->current_values != this->state_->remote_values) {
    ESP_LOGD(UyatLightRGBCT::TAG, "Light is transitioning, datapoint change ignored");
    return;
  }
...
void UyatLightRGBCT::on_switch_value(const bool value)
{
  ESP_LOGV(UyatLightRGBCT::TAG, "MCU reported switch %s is: %s", this->get_name().c_str(), ONOFF(value));
  if (this->state_->current_values != this->state_->remote_values) {
    ESP_LOGD(UyatLightRGBCT::TAG, "Light is transitioning, datapoint change ignored");
    return;
  }
...
void UyatLightRGBCT::on_color_value(const DpColor::Value& value)
{
  ESP_LOGV(UyatLightRGBCT::TAG, "MCU reported color %s is: %.2f, %.2f, %.2f", this->get_name().c_str(), value.r, value.g, value.b);
...
void UyatLightRGBCT::on_white_temperature_value(const float value_percent)
{
  ESP_LOGV(UyatLightRGBCT::TAG, "Dimmer of %s reported white temperature: %.4f", this->get_name().c_str(), value_percent);
  if (this->state_->current_values != this->state_->remote_values) {
    ESP_LOGD(UyatLightRGBCT::TAG, "Light is transitioning, datapoint change ignored");
    return;
  }
